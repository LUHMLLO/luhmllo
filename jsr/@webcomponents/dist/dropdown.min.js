...
var Ct = async (t, e, n) => {
  ...
  for (let p = 0; p < s.length; p++) {
    let { name: y, fn: g } = s[p],
      { x: w, y: v, data: b, reset: x } = await g({
        x: u,
        y: d,
        initialPlacement: i,
        placement: m,
    ...
    async fn(e) {
      ...
      let K = x.map((R) => {
        ...
        U = ((r = K.filter((R) =>
          ...
          : r[0]) || K[0][0];
      return U !== s
        ? { data: { index: g + 1, overflows: x }, reset: { placement: U } }
        : {};
    },
  };
};
  ...
  return t === void 0 && (t = {}), {
    ...
    async fn(e) {
      ...
      let y = f - l.top - l.bottom,
        ...
        x = w,
        $ = v;
      if (d ? $ = u || b ? _(v, g) : g : x = u || b ? _(w, y) : y, b && !u) {
        let U = H(l.left, 0),
          R = H(l.right, 0),
          D = H(l.top, 0),
          tt = H(l.bottom, 0);
        d
          ...
          : x = f - 2 * (D !== 0 || tt !== 0 ? D + tt : H(l.top, l.bottom));
      }
      await o({ ...e, availableWidth: $, availableHeight: x });
  ...
  if (a) {
    ...
    for (; p && i && f !== h;) {
      let y = X(p),
        ...
        w = O(p),
        v = g.left + (p.clientLeft + parseFloat(w.paddingLeft)) * y.x,
...
function jt(t, e) {
  ...
  let i = Mt(t, e);
  for (; i && vt(i) && dt(i);) i = Mt(i, e);
  return i && N(i) && dt(i) && !it(i) ? n : i || xt(t) || n;
}
...
function ye(t, e) {
  ...
  function o(s, l) {
    ...
    try {
      n = new IntersectionObserver(v, { ...g, root: r.ownerDocument });
    } catch {
